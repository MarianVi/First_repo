"""
Observer este un design pattern comportamental care te lasa sa definesti un mecanism de subscriere prin care
    mai multe obiecte pot fi notificate despre evenimente care se intampla in obiectele pe care le observa
"""
...
class Person(Observer):
    ...
    def update(self, observable):
        if observable.istoric[-1].content.startswith(self.name):
            ...
            response = input('Vrei sa raspunzi? (y / n) ').lower()
            if response == 'y':
